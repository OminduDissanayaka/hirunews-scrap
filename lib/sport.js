...
/**
 * Sport section extractor — fetches the first main article from the
 * sports listing and returns a standardized article object.
 */
...
/**
 * Extract article data from the sports listing using the provided selector.
 *
 * @param {string} urlSelector - CSS selector locating the article link
 * @returns {Promise<object>} Article details
 * @throws {ValidationError|Error}
 */
async function extractNewsData(urlSelector) {
    try {
        const baseUrl = 'https://www.hirunews.lk/sports/';
        ...
        logError(error, 'sport.extractNewsData');
        throw error;
    }
}
/**
 * Public: get the latest sports article.
